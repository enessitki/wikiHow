...
class Window(QWidget):
    ...
    def obstacle_btn_clicked(self):
        ...
        self.open_set.clear()
        start_id = self.calc_index(self.start_node)
        self.open_set[start_id] = self.start_node
    ...
    def is_goal_closer_than_step(self, node):
        # result = None
        # min_dist_id = min(self.open_set,
        #                   key=lambda o: self.geodesic.measure((self.open_set[o].x, self.open_set[o].y),
        ...
        #     result = min_dist_id
        dist = self.geodesic.measure((node.x, node.y), (self.goal_node.x, self.goal_node.y))*1000
        result = False
    ...
    def calc_neighbor(self, n1, i):
        ...
        cost = n1.cost + step
        result = [x, y, cost]
        print("Neighbor", i, ":", result)
        return [x, y, cost]

    def is_in_obstacle(self, n1):
        result = False
        p1 = Point(n1.x, n1.y)
        for poly in self.obstacleList:
    ...
    def planning(self):
        ...
        # newGoalNodeParentIndex = self.is_goal_closer_than_step(current)
        #
        ...
        if self.is_goal_closer_than_step(current):
            self.goal_node.parent_index = c_id
            self.calc_final_path()
