...
class SickS300:
    def __init__(self, max_distance=1000, start_angle=-45, end_angle=225):
    ...
    def measurement_parser(self, measurement):
        ...
        while count < N-2:
            ...
            value /= 100
            if value < self.maxDistance and self.startAngle <= math.degrees(angle) < self.endAngle:
                x = value * math.cos(angle)
                z = value * math.sin(angle)
                values[math.degrees(angle)] = ([x, 0, z])
            count += 2
