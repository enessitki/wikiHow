...
class UDPCommunication:
    ...
    def udp(self):
        ...
        while True:
            ...
            lat = UDPCommunication.coordinate_32_convert_to_int(data[0:4])
            lon = UDPCommunication.coordinate_32_convert_to_int(data[4:8])
            mod = data[8]

            if mod == 0:
                path_elimination.eliminate_path(lat, lon)
                pass
            elif mod == 1:
                coordinates = path_elimination.coordinates
                l2 = len(coordinates)
                last_lat, last_lon = coordinates[l2-1]
